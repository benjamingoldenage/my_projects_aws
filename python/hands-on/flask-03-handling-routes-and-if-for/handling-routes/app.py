...
def admin():
    ...
    # istediğin sayfayı yönlendrebilirsin. 
...
# Create a function named greet_admin which redirect the request to the greet function with param of 'Master Admin!!!!' 
# and assign to the route of ('/greet-admin')

@app.route('/greet_admin')
def greet_admin():
    return redirect(url_for('greet',name = 'Master_Admin !!!'))



# Rewrite a function named greeting which which uses template file named `greet.html` under `templates` folder 
# and assign to the dynamic route of ('/<name>')
@app.route('/<name>')
def greeting(name):
