...
def main_post():
    if request.method == 'POST':
        ...
        if not alpha.isdecimal():
            return render_template('index.html', developer_name='Benjamin' , not_valid=True)
        number=int(alpha)
        if not 0 < number < 4000:
            return render_template('index.html', developer_name='Benjamin' , not_valid=True)
        return render_template('result.html', number_decimal=number, number_roman= convert(number), developer_name='Benjamin')
    else:
        return render_template('index.html' , developer_name= 'Benjamin', not_valid=False)
